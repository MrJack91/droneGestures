...
\todo{correct it}
Da Drohnen während der letzten Zeit sehr aufgekommen und erschwinglich wurden, lassen sich sehr viele Drohnen-Projekte mit jeweils verschiedenster Hardware finden.
...
Für viele der gefundenen Projekte wurde nebst der Crazyflie auch die \textit{AR.drone} von Parrot verwendet.\footcite{AR_Drone_2.0_Parrot_2015-04-29}
Die AR.drone ist erheblich grösser als die Crazyflie, was viel zur Stabilität im Flug beiträgt. 
...
Die Crazyflie richtet sich hauptsächlich an Entwickler, während die AR.drone auch normal Nutzer anspricht.
Darum wurde in dieser Arbeit die Crazyflie verwendet.

\subsubsection{Steuermöglichkeiten}
Zusammengefasst können die implementierten Steuerungen in folgende Kategorien eingeteilt werden:
\begin{itemize}

	\item \textbf{Steuerungen mit der offenen Hand:} Bei einer offenen Handsteuerung basiert die Steuerung auf der Erkennung der offenen Hand. Meist befindet sich der Sensor für die Gestenerkennung unterhalb der Hand und kann so stabil und mobil aufgestellt werden.
	
		\begin{itemize}
			
			\item \textbf{Handrichtungs-basierte Steuerung:} Diese Steuerung ist an der Benutzung eines gewohnten Hebels oder Joystick angelehnt. Die Drohne bewegt sich analog der Hand. Dabei spielt der Winkel der Hand keine Rolle.
			Beispielweise bedeutete eine Hand die vor dem Zentrum des Sensores liegt, dass die Drohne nach vorne fliegen soll. Das Pitch Manöver wird oft durch spezielle Gesten wie \textit{Kreisen} umgesetzt.
			...
			Bei der handpositions-basierten Steuerung, dient die Position der Hand, effektiv der Position der Drohne. Der Winkel der Handhaltung, soll des Winkels der Drohne entsprechen.
			Wo genau sich die Drohne über dem Sensor befindet, ist ausser für die Höhe (Thrust), nicht relevant.
			Da diese Steuerungsart der Intuitivsten entspricht, wird in dieser Arbeit diese Art Steuerung implementiert.
	...
	Bei dieser Steuerung erfolgen die Kommandos wie bei einem Lenkdrachen, also wie wenn die Drohne an drei Eisenstangen befestigt wäre und man an einer zieht, um die Position der Drohne zu verändern.
	Um diese Gesten zu erkenne, wird der Sensor oder die Kamera frontal befestigt, z.B. am Bildschirmrand.
	Bei einer Implementation am Hackathon in München, wird als Startzeichen das heben der Daumen verwendet.\footcite{Drones_Fly_Hands_Free_with_Gestural_Technology_2015-04-29}
	...
	Da besonders in jüngster Zeit die \glspl{glos:wearableLabel} sehr oft erwähnt werden, wird sich deren Nutzung in Zukunft bestimmt stark vermehren.
	Momentan sind \glspl{glos:wearableLabel} den Gestenerkennungssensoren noch unterlegen.
	Trotzdem ein Beispiel von den Hackdays.\footcite{Wearable_Gesture_Controlled_Drone_Hackaday_2015-04-29}
\end{itemize}
